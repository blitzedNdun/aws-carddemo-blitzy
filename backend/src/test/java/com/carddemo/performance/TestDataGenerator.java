...
public class TestDataGenerator {
    ...
    public Account generateAccount() {
        ...
        // Generate account ID using Constants.ACCOUNT_ID_LENGTH constraint
        account.setAccountId(generateAccountId());
        
        // Generate customer ID using Constants.CUSTOMER_ID_LENGTH constraint  
        ...
        account.setCashCreditLimit(creditLimit.multiply(new BigDecimal("0.25")));
    ...
    public Transaction generateTransaction() {
        ...
        // Generate transaction ID using Constants.TRANSACTION_ID_LENGTH
        transaction.setTransactionId(generateTransactionId());
        
        // Generate account ID for relationship
        ...
        LocalDateTime transactionDate = generateTransactionDate();
        transaction.setTransactionDate(transactionDate);
        transaction.setOrigTranDate(transactionDate);
        transaction.setProcessedDate(transactionDate.plusMinutes(random.nextInt(60)));
    ...
    public Customer generateCustomer() {
        ...
        // Generate customer ID using Constants.CUSTOMER_ID_LENGTH
        ...
        customer.setCustomerPhone(generateRandomPhoneNumber());
        customer.setCustomerEmail(generateEmail(firstName, lastName));
        
        // Generate address information
        String[] address = generateRandomAddress();
        customer.setCustomerAddrLine1(address[0]);
        customer.setCustomerCity(address[1]);
        customer.setCustomerState(address[2]);
        customer.setCustomerZip(address[3]);
        customer.setCustomerCountryCode("US");
    ...
    public Card generateCard() {
        ...
        // Set card status
        String activeStatus = random.nextBoolean() ? "ACTIVE" : "INACTIVE";
        card.setActiveStatus(activeStatus);
        
        // Set additional card fields
        card.setCardType("CREDIT");
        card.setEmbossedName(FIRST_NAMES[random.nextInt(FIRST_NAMES.length)] + " " + 
                           LAST_NAMES[random.nextInt(LAST_NAMES.length)]);
        card.setDateIssued(LocalDate.now().minusDays(random.nextInt(365)));
    ...
    public BigDecimal generateComp3BigDecimal(int integerDigits, int decimalPlaces, double maxValue) {
        ...
        // Use CobolDataConverter to preserve precision
        return CobolDataConverter.preservePrecision(result, integerDigits, decimalPlaces);
    }
    ...
    public String generateValidCardNumber() {
        ...
        if (ValidationUtil.validateCardNumber(cardNumberStr)) {
            return cardNumberStr;
        } else {
            // Retry if validation fails
            return generateValidCardNumber();
        }
    }
    ...
    public String generateValidSSN() {
        ...
        // Validate using ValidationUtil
        if (ValidationUtil.validateSSN(ssnStr)) {
            return ssnStr;
        } else {
            // Retry if validation fails
    ...
    public String generateRandomPhoneNumber() {
        ...
        // Validate area code using ValidationUtil
        if (ValidationUtil.validatePhoneAreaCode(areaCode)) {
            ...
            // Retry with different area code
    ...
    public String[] generateRandomAddress() {
        ...
        if (ValidationUtil.validateUSStateCode(stateCode)) {
            ...
            // Fallback to default valid state
            address[2] = "CA";
        }
    ...
    /**
     * Generates customer ID matching Constants.CUSTOMER_ID_LENGTH constraint
     */
    private String generateCustomerId() {
        return generateNumericId(Constants.CUSTOMER_ID_LENGTH);
    }
