...
-- =============================================================================
-- 10. AUTHORIZATION DATA TABLE
-- =============================================================================
-- Maps to Authorization entity for transaction authorization processing
-- Includes merchant_id field for transaction processing support
CREATE TABLE authorization_data (
    ...
    merchant_id BIGINT,
    CONSTRAINT fk_auth_card FOREIGN KEY (card_number) REFERENCES card_data(card_number),
    CONSTRAINT fk_auth_account FOREIGN KEY (account_id) REFERENCES account_data(account_id),
    CONSTRAINT chk_approval_status CHECK (approval_status IN ('PENDING', 'APPROVED', 'DECLINED'))
);

-- Test-optimized indexes for authorization operations
CREATE INDEX idx_authorization_card ON authorization_data (card_number);
CREATE INDEX idx_authorization_account ON authorization_data (account_id);
CREATE INDEX idx_authorization_status ON authorization_data (approval_status);

-- =============================================================================
-- 11. CARD CROSS-REFERENCE TABLE
-- =============================================================================
...
INSERT INTO authorization_data (card_number, account_id, transaction_amount, authorization_code, approval_status, decline_reason_code, processing_time, fraud_score, velocity_check_result, merchant_id) VALUES
('4111111111111111', 11111111111, 125.50, 'AUTH01', 'APPROVED', NULL, 250, 15.5, false, 1001),
('4222222222222222', 22222222222, 89.25, 'AUTH02', 'APPROVED', NULL, 180, 8.2, false, 1002),
('4333333333333333', 33333333333, 25.00, 'AUTH03', 'APPROVED', NULL, 320, 12.3, false, 1003),
('4444444444444444', 44444444444, 250.75, 'AUTH04', 'DECLINED', 'INSU', 150, 45.8, true, 1004),
('4555555555555555', 55555555555, 1000.00, 'AUTH05', 'APPROVED', NULL, 200, 5.1, false, 1005);
...
BEGIN
    ...
    ELSE
        RAISE EXCEPTION 'FAILURE: Only % tables created, expected at least 11', table_count;
    END IF;
END;
$$;
