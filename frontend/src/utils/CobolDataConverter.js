 ...
 */
export function fromComp3(packedData, scale = 0, precision = 15) {
  if (!packedData) {
  ...
  // Remove leading zeros but preserve at least one digit
...
export function toComp3(value, scale = 0, precision = 15) {
  ...
  // If odd number of digits, start with a leading zero
  if (precision % 2 === 0) {
    digits = '0' + digits;
  }
  ...
  for (let i = 0; i < byteCount - 1; i++) {
    const leftDigit = parseInt(digits[digitIndex], 10);
    const rightDigit = parseInt(digits[digitIndex + 1], 10);
    packedBytes[i] = (leftDigit << 4) | rightDigit;
    digitIndex += 2;
  }

  // Pack the last byte with final digit and sign
  const finalDigit = parseInt(digits[digitIndex], 10);
...
export function formatCobolNumber(value, picClause, impliedDecimal = false) {
  ...
  // Round to integer
  const integerValue = scaledValue.toFixed(0, COBOL_ROUNDING_MODES.HALF_UP);
  const absoluteValue = Math.abs(parseInt(integerValue));
  
  // Format with leading zeros
  let formatted = absoluteValue.toString().padStart(picInfo.totalDigits, '0');
 ...
 */
export function formatDecimal(value, scale = 2, roundingMode = COBOL_ROUNDING_MODES.HALF_UP, stripTrailingZeros = false) {
  const decimal = new Decimal(value || 0);
