...
export const validateDate = (dateString) => {
  ...
  const century = parseInt(dateString.substring(0, 2));
  ...
  if (!thirtyOneDayMonths.includes(month) && day === 31) {
    result.isValid = false;
    result.errorMessage = 'Cannot have 31 days in this month.';
    return result;
  }
  ...
  if (month === 2) {
    if (day === 30) {
      result.isValid = false;
      result.errorMessage = 'Cannot have 30 days in this month.';
      return result;
    }
    
    // Leap year logic for February 29th (matching COBOL logic)
    if (day === 29) {
      ...
      if (year === 0) {
        ...
        isLeapYear = (fullYear % 400 === 0);
      } else {
        // Non-century year - divisible by 4
        isLeapYear = (fullYear % 4 === 0);
      }
      
      if (!isLeapYear) {
        result.isValid = false;
        result.errorMessage = 'Not a leap year. Cannot have 29 days in this month.';
        return result;
      }
    }
  }
...
export const validateSSN = (ssn) => {
  ...
  if (area === '000' || area === '666' || (parseInt(area) >= 900 && parseInt(area) <= 999)) {
    result.isValid = false;
...
export const createStateZipValidationSchema = (stateFieldName = 'State', zipFieldName = 'ZIP Code') => {
  return {
    ...
    test: function(value, context) {
      ...
      if (!stateValue || !zipValue) return false;
