...
SELECT
    ...
    -- Parse fields using "{" delimiter from position 22 onward
    -- current_balance: field 1 after position 21
    CAST(split_part(SUBSTRING(tar.raw_data, 22), '{', 1) AS BIGINT) as current_balance_cents,
    -- credit_limit: field 2
    CAST(split_part(SUBSTRING(tar.raw_data, 22), '{', 2) AS BIGINT) as credit_limit_cents,
    -- cash_credit_limit: field 3
    CAST(split_part(SUBSTRING(tar.raw_data, 22), '{', 3) AS BIGINT) as cash_credit_limit_cents,
    -- Field 4 contains dates + current_cycle_credit concatenated: dates(30) + current_cycle_credit(11)
    -- Extract dates from first 30 characters of field 4
    SUBSTRING(split_part(SUBSTRING(tar.raw_data, 22), '{', 4), 1, 10) as open_date_str,
    SUBSTRING(split_part(SUBSTRING(tar.raw_data, 22), '{', 4), 11, 10) as expiration_date_str,
    SUBSTRING(split_part(SUBSTRING(tar.raw_data, 22), '{', 4), 21, 10) as reissue_date_str,
    -- Extract current_cycle_credit from last 11 characters of field 4
    CAST(RIGHT(split_part(SUBSTRING(tar.raw_data, 22), '{', 4), 11) AS BIGINT) as current_cycle_credit_cents,
    -- current_cycle_debit: field 5
    CAST(split_part(SUBSTRING(tar.raw_data, 22), '{', 5) AS BIGINT) as current_cycle_debit_cents,
    -- group_id: field 6 - trim whitespace
    TRIM(split_part(SUBSTRING(tar.raw_data, 22), '{', 6)) as group_id
