-- =====================================================================================
-- Liquibase Migration: V22__load_accounts_data.sql
-- Description: Loads account data from acctdata.txt ASCII source with precise 
--              financial field mapping, customer relationships, and disclosure group 
--              associations supporting comprehensive account management operations
-- Author: Blitzy Agent
-- Date: 2024
-- Version: 22.0
-- Dependencies: V3__create_accounts_table.sql, V21__load_customers_data.sql, V6__create_reference_tables.sql
-- =====================================================================================

-- changeset blitzy:V22-load-accounts-data
-- comment: Load account data from acctdata.txt preserving exact 11-digit account_id and DECIMAL(12,2) precision for financial integrity

-- =============================================================================
-- 1. CREATE TEMPORARY STAGING TABLE FOR RAW DATA PROCESSING
-- =============================================================================
-- Create temporary table for staging acctdata.txt raw data during processing
CREATE TEMPORARY TABLE temp_accounts_raw (
    raw_record TEXT
);

-- Create temporary table for parsed account data with proper data types
CREATE TEMPORARY TABLE temp_accounts_parsed (
    account_id VARCHAR(11),
    customer_id VARCHAR(9),
    active_status VARCHAR(1),
    current_balance_raw VARCHAR(12),
    credit_limit_raw VARCHAR(12),
    cash_credit_limit_raw VARCHAR(12),
    open_date DATE,
    expiration_date DATE,
    reissue_date DATE,
    current_cycle_credit_raw VARCHAR(12),
    current_cycle_debit_raw VARCHAR(12),
    group_id VARCHAR(10),
    current_balance DECIMAL(12,2),
    credit_limit DECIMAL(12,2),
    cash_credit_limit DECIMAL(12,2),
    current_cycle_credit DECIMAL(12,2),
    current_cycle_debit DECIMAL(12,2)
);

-- =============================================================================
-- 2. LOAD RAW ACCOUNT DATA FROM acctdata.txt
-- =============================================================================
-- Load the 50 account records from acctdata.txt preserving exact format
INSERT INTO temp_accounts_raw (raw_record) VALUES 
('00000000001Y00000001940{00000020200{00000010200{2014-11-202025-05-202025-05-2000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000002Y00000001580{00000061300{00000054480{2013-06-192024-08-112024-08-1100000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000003Y00000001470{00000049090{00000005380{2013-08-232024-01-102024-01-1000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000004Y00000000400{00000035030{00000027890{2012-11-172023-12-162023-12-1600000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000005Y00000003450{00000038190{00000024300{2012-10-032025-03-092025-03-0900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000006Y00000002180{00000035840{00000029480{2017-12-232025-10-082025-10-0800000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000007Y00000001930{00000020650{00000002640{2012-10-122024-12-132024-12-1300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000008Y00000006050{00000061040{00000013180{2012-01-042024-05-202024-05-2000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000009Y00000005600{00000082010{00000020650{2016-08-272024-12-272024-12-2700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000010Y00000001590{00000054010{00000044420{2015-09-132023-01-272023-01-2700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000011Y00000002120{00000049980{00000031750{2014-09-122025-03-122025-03-1200000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000012Y00000001760{00000046360{00000003880{2009-06-172023-07-072023-07-0700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000013Y00000000410{00000075420{00000049220{2017-10-012024-08-042024-08-0400000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000014Y00000000150{00000022540{00000002120{2010-12-042025-12-112025-12-1100000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000015Y00000004890{00000084410{00000038330{2009-10-062025-06-092025-06-0900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000016Y00000007330{00000089220{00000026320{2014-09-112024-01-252024-01-2500000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000017Y00000000330{00000005680{00000005100{2014-05-172025-03-012025-03-0100000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000018Y00000001440{00000029030{00000014960{2018-11-152023-09-102023-09-1000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000019Y00000004800{00000069860{00000037230{2011-12-142025-07-232025-07-2300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000020Y00000003690{00000037670{00000010400{2014-02-272024-03-132024-03-1300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000021Y00000001120{00000012640{00000001800{2011-10-192023-01-062023-01-0600000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000022Y00000000550{00000085990{00000047120{2016-11-212025-12-282025-12-2800000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000023Y00000001040{00000033770{00000029040{2012-03-152025-03-182025-03-1800000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000024Y00000004000{00000051740{00000041290{2015-08-082025-02-112025-02-1100000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000025Y00000000610{00000081940{00000065820{2012-10-262025-07-102025-07-1000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000026Y00000000460{00000021810{00000013750{2009-04-202024-12-192024-12-1900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000027Y00000002840{00000055720{00000020750{2012-09-302025-07-132025-07-1300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000028Y00000000680{00000008680{00000005470{2015-05-202024-05-092024-05-0900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000029Y00000003390{00000055110{00000043610{2015-11-032024-06-042024-06-0400000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000030Y00000000020{00000001200{00000000930{2011-08-262024-06-272024-06-2700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000031Y00000000310{00000011400{00000010770{2017-02-252025-06-082025-06-0800000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000032Y00000000300{00000011750{00000008460{2013-11-102025-05-192025-05-1900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000033Y00000004100{00000064040{00000009510{2012-10-112025-10-072025-10-0700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000034Y00000002530{00000036420{00000027700{2009-05-102025-10-062025-10-0600000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000035Y00000001660{00000019470{00000015250{2018-02-022025-09-232025-09-2300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000036Y00000001100{00000033280{00000008390{2018-07-182024-12-232024-12-2300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000037Y00000000070{00000004460{00000001660{2016-09-102023-10-242023-10-2400000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000038Y00000006120{00000065050{00000034760{2010-08-122023-07-232023-07-2300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000039Y00000008430{00000097500{00000062120{2018-08-262025-09-082025-09-0800000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000040Y00000000430{00000058230{00000016740{2010-02-132023-10-272023-10-2700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000041Y00000003750{00000067210{00000034290{2015-02-072023-04-242023-04-2400000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000042Y00000003020{00000065630{00000051030{2016-09-192025-09-192025-09-1900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000043Y00000006100{00000061680{00000012060{2012-04-092025-08-292025-08-2900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000044Y00000002630{00000068990{00000044320{2018-12-012024-01-172024-01-1700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000045Y00000001860{00000027190{00000006880{2010-12-312025-07-092025-07-0900000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000046Y00000003960{00000070070{00000054380{2013-09-062025-06-202025-06-2000000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000047Y00000000320{00000023380{00000001590{2014-04-032025-08-232025-08-2300000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000048Y00000002260{00000023060{00000006120{2017-03-182025-02-062025-02-0600000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000049Y00000001000{00000090480{00000048070{2019-04-062023-09-172023-09-1700000000000{00000000000{A000000000                                                                                                                                                                                            '),
('00000000050Y00000004920{00000061690{00000045870{2011-04-222023-03-092023-03-0900000000000{00000000000{A000000000                                                                                                                                                                                            ');

-- =============================================================================
-- 3. PARSE RAW ACCOUNT DATA WITH PRECISE FIELD EXTRACTION
-- =============================================================================
-- Parse the fixed-width format preserving exact 11-digit account_id structure
INSERT INTO temp_accounts_parsed (
    account_id,
    customer_id,
    active_status,
    current_balance_raw,
    credit_limit_raw,
    cash_credit_limit_raw,
    open_date,
    expiration_date,
    reissue_date,
    current_cycle_credit_raw,
    current_cycle_debit_raw,
    group_id,
    current_balance,
    credit_limit,
    cash_credit_limit,
    current_cycle_credit,
    current_cycle_debit
)
SELECT 
    -- Extract 11-digit account_id preserving exact format
    SUBSTR(raw_record, 1, 11) as account_id,
    
    -- Extract 9-digit customer_id for foreign key relationship
    SUBSTR(raw_record, 13, 9) as customer_id,
    
    -- Extract active status indicator (Y/N)
    SUBSTR(raw_record, 12, 1) as active_status,
    
    -- Extract raw monetary values with '{' delimiter parsing
    SUBSTR(raw_record, 22, 12) as current_balance_raw,
    SUBSTR(raw_record, 34, 12) as credit_limit_raw,
    SUBSTR(raw_record, 46, 12) as cash_credit_limit_raw,
    
    -- Extract and parse ISO format dates
    SUBSTR(raw_record, 58, 10)::DATE as open_date,
    SUBSTR(raw_record, 68, 10)::DATE as expiration_date,
    SUBSTR(raw_record, 78, 10)::DATE as reissue_date,
    
    -- Extract raw current cycle values with '{' delimiter parsing
    SUBSTR(raw_record, 88, 12) as current_cycle_credit_raw,
    SUBSTR(raw_record, 100, 12) as current_cycle_debit_raw,
    
    -- Extract group_id for disclosure group association
    SUBSTR(raw_record, 112, 10) as group_id,
    
    -- Convert COBOL COMP-3 format to DECIMAL(12,2) precision
    -- Replace '{' with '.' and convert to proper decimal format
    (REPLACE(SUBSTR(raw_record, 22, 12), '{', '.'))::DECIMAL(12,2) as current_balance,
    (REPLACE(SUBSTR(raw_record, 34, 12), '{', '.'))::DECIMAL(12,2) as credit_limit,
    (REPLACE(SUBSTR(raw_record, 46, 12), '{', '.'))::DECIMAL(12,2) as cash_credit_limit,
    (REPLACE(SUBSTR(raw_record, 88, 12), '{', '.'))::DECIMAL(12,2) as current_cycle_credit,
    (REPLACE(SUBSTR(raw_record, 100, 12), '{', '.'))::DECIMAL(12,2) as current_cycle_debit
FROM temp_accounts_raw;

-- =============================================================================
-- 4. DATA VALIDATION AND REFERENTIAL INTEGRITY CHECKS
-- =============================================================================
-- Validate that all customer_id references exist in customers table
DO $$
DECLARE
    missing_customers_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO missing_customers_count
    FROM temp_accounts_parsed tap
    LEFT JOIN customers c ON tap.customer_id = c.customer_id
    WHERE c.customer_id IS NULL;
    
    IF missing_customers_count > 0 THEN
        RAISE EXCEPTION 'Data integrity violation: % account records reference non-existent customers', missing_customers_count;
    END IF;
END $$;

-- Validate that all group_id references exist in disclosure_groups table
DO $$
DECLARE
    missing_groups_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO missing_groups_count
    FROM temp_accounts_parsed tap
    LEFT JOIN disclosure_groups dg ON TRIM(tap.group_id) = dg.group_id
    WHERE dg.group_id IS NULL;
    
    IF missing_groups_count > 0 THEN
        RAISE EXCEPTION 'Data integrity violation: % account records reference non-existent disclosure groups', missing_groups_count;
    END IF;
END $$;

-- Validate account_id uniqueness and 11-digit format
DO $$
DECLARE
    invalid_account_ids_count INTEGER;
    duplicate_account_ids_count INTEGER;
BEGIN
    -- Check for invalid account_id format
    SELECT COUNT(*) INTO invalid_account_ids_count
    FROM temp_accounts_parsed
    WHERE account_id !~ '^[0-9]{11}$';
    
    IF invalid_account_ids_count > 0 THEN
        RAISE EXCEPTION 'Data format violation: % account records have invalid 11-digit account_id format', invalid_account_ids_count;
    END IF;
    
    -- Check for duplicate account_id values
    SELECT COUNT(*) INTO duplicate_account_ids_count
    FROM (
        SELECT account_id, COUNT(*) as cnt
        FROM temp_accounts_parsed
        GROUP BY account_id
        HAVING COUNT(*) > 1
    ) duplicates;
    
    IF duplicate_account_ids_count > 0 THEN
        RAISE EXCEPTION 'Data integrity violation: % duplicate account_id values found', duplicate_account_ids_count;
    END IF;
END $$;

-- =============================================================================
-- 5. LOAD VALIDATED DATA INTO ACCOUNTS TABLE
-- =============================================================================
-- Insert parsed and validated account data maintaining exact financial precision
INSERT INTO accounts (
    account_id,
    customer_id,
    active_status,
    current_balance,
    credit_limit,
    cash_credit_limit,
    open_date,
    expiration_date,
    reissue_date,
    current_cycle_credit,
    current_cycle_debit,
    group_id,
    -- Optional fields with defaults
    address_zip,
    created_at,
    updated_at
)
SELECT 
    tap.account_id,
    tap.customer_id,
    tap.active_status,
    tap.current_balance,
    tap.credit_limit,
    tap.cash_credit_limit,
    tap.open_date,
    tap.expiration_date,
    tap.reissue_date,
    tap.current_cycle_credit,
    tap.current_cycle_debit,
    TRIM(tap.group_id) as group_id,
    -- Set address_zip to NULL for data loading (will be populated separately)
    NULL as address_zip,
    CURRENT_TIMESTAMP as created_at,
    CURRENT_TIMESTAMP as updated_at
FROM temp_accounts_parsed tap
ORDER BY tap.account_id;

-- =============================================================================
-- 6. POST-LOAD VALIDATION AND STATISTICS
-- =============================================================================
-- Validate successful data loading with comprehensive statistics
DO $$
DECLARE
    loaded_count INTEGER;
    expected_count INTEGER := 50;
    total_balance DECIMAL(15,2);
    total_credit_limit DECIMAL(15,2);
    active_accounts INTEGER;
    inactive_accounts INTEGER;
BEGIN
    -- Count loaded records
    SELECT COUNT(*) INTO loaded_count FROM accounts;
    
    -- Validate expected record count
    IF loaded_count < expected_count THEN
        RAISE EXCEPTION 'Data loading incomplete: Expected % records, loaded % records', expected_count, loaded_count;
    END IF;
    
    -- Calculate comprehensive statistics
    SELECT 
        SUM(current_balance),
        SUM(credit_limit),
        SUM(CASE WHEN active_status = 'Y' THEN 1 ELSE 0 END),
        SUM(CASE WHEN active_status = 'N' THEN 1 ELSE 0 END)
    INTO total_balance, total_credit_limit, active_accounts, inactive_accounts
    FROM accounts;
    
    -- Log loading statistics
    RAISE NOTICE 'Account data loading completed successfully:';
    RAISE NOTICE '  Total accounts loaded: %', loaded_count;
    RAISE NOTICE '  Active accounts: %', active_accounts;
    RAISE NOTICE '  Inactive accounts: %', inactive_accounts;
    RAISE NOTICE '  Total current balance: $%.2f', total_balance;
    RAISE NOTICE '  Total credit limit: $%.2f', total_credit_limit;
    RAISE NOTICE '  Average balance per account: $%.2f', total_balance / loaded_count;
    RAISE NOTICE '  Average credit limit per account: $%.2f', total_credit_limit / loaded_count;
END $$;

-- =============================================================================
-- 7. FOREIGN KEY CONSTRAINT ACTIVATION
-- =============================================================================
-- Activate foreign key constraint to disclosure_groups table
-- (This constraint was commented out in the table creation script)
ALTER TABLE accounts 
ADD CONSTRAINT accounts_group_id_fkey 
FOREIGN KEY (group_id) REFERENCES disclosure_groups(group_id)
ON DELETE RESTRICT ON UPDATE CASCADE;

-- =============================================================================
-- 8. REFRESH MATERIALIZED VIEW FOR ACCOUNT SUMMARY
-- =============================================================================
-- Refresh the materialized view to include new account data
REFRESH MATERIALIZED VIEW mv_account_summary;

-- =============================================================================
-- 9. FINAL INTEGRITY VERIFICATION
-- =============================================================================
-- Perform final integrity checks to ensure data consistency
DO $$
DECLARE
    integrity_issues INTEGER := 0;
    orphaned_accounts INTEGER;
    invalid_balances INTEGER;
    date_inconsistencies INTEGER;
BEGIN
    -- Check for orphaned accounts (should be 0 due to foreign key constraints)
    SELECT COUNT(*) INTO orphaned_accounts
    FROM accounts a
    LEFT JOIN customers c ON a.customer_id = c.customer_id
    WHERE c.customer_id IS NULL;
    
    -- Check for accounts with invalid balance constraints
    SELECT COUNT(*) INTO invalid_balances
    FROM accounts
    WHERE current_balance < -99999999.99 
       OR current_balance > 99999999.99
       OR credit_limit < 0
       OR cash_credit_limit < 0
       OR cash_credit_limit > credit_limit;
    
    -- Check for date inconsistencies
    SELECT COUNT(*) INTO date_inconsistencies
    FROM accounts
    WHERE expiration_date <= open_date
       OR reissue_date < open_date;
    
    integrity_issues := orphaned_accounts + invalid_balances + date_inconsistencies;
    
    IF integrity_issues > 0 THEN
        RAISE EXCEPTION 'Data integrity verification failed: % issues found (orphaned: %, invalid balances: %, date issues: %)', 
                       integrity_issues, orphaned_accounts, invalid_balances, date_inconsistencies;
    END IF;
    
    RAISE NOTICE 'Data integrity verification completed successfully - no issues found';
END $$;

-- =============================================================================
-- 10. CLEANUP TEMPORARY TABLES
-- =============================================================================
-- Clean up temporary tables used during data loading
DROP TABLE IF EXISTS temp_accounts_parsed;
DROP TABLE IF EXISTS temp_accounts_raw;

-- Add final comment with loading summary
COMMENT ON TABLE accounts IS 'Account master data table migrated from VSAM ACCTDAT dataset - 50 records loaded from acctdata.txt with precise financial field mapping and customer relationships';

-- rollback changeset blitzy:V22-load-accounts-data
-- DELETE FROM accounts WHERE account_id IN (SELECT account_id FROM accounts ORDER BY account_id LIMIT 50);
-- ALTER TABLE accounts DROP CONSTRAINT IF EXISTS accounts_group_id_fkey;
-- REFRESH MATERIALIZED VIEW mv_account_summary;